...
const CORE_BLOCKS = new Set([
  "summary", "decision", "constraint", "task", "file",
  "example", "note", "warning", "api", "link",
]);
...
function parseHeading(line: string, lineNumber: number) {
  ...
  return { type: "heading" as const, level: match[1].length, text: match[2].trimEnd() };
}
function parseBlock(lines: string[], startIndex: number) {
  ...
  return { node: { type: "block" as const, name, attrs, text: body.join("\n").trim() }, nextIndex: index };
}
...
export type LessmarkAst = {
  ...
  children: Array<
    ...
    | { type: "block"; name: string; attrs: Record<string, string>; text: string }
  >;
};

export function parseLessmark(source: string): LessmarkAst {
  ...
  while (index < lines.length) {
    ...
    if (line.startsWith("#")) { children.push(parseHeading(line, index + 1)); index += 1; continue; }
    if (line.startsWith("@")) { const p = parseBlock(lines, index); children.push(p.node); index = p.nextIndex; continue; }
